...
#define dmac_region(daddr)  dmac->io[daddr]
    ...
    uint32_t direction)
{
    ...
    dma_msg->host_data = 0;
    dma_msg->src = dmac_region(DGBRP(chan));
    dma_msg->dest = dmac_region(DGBWP(chan));
    dma_msg->size = dmac_region(DGBFPI(chan));
    dma_msg->dmac_id = dmac->id;
    dma_msg->chan_id = chan;
    dma_msg->client_data = (uint64_t)hda_dma_chan;
    dma_msg->direction = direction;

    //trace_hda_dma_m2m_req(dma_msg->src, dma_msg->dest, dma_msg->size);
...
static void dma_host_complete(struct adsp_hda_dmac *dmac, uint32_t chan)
{
    ...
    //trace_hda_dma_m2m_complete(dma_msg->src, dma_msg->dest, dma_msg->size);
...
{
    dmac_region(DGBRP(chan)) += bytes;

    /* check R ptr wrt to W ptr */
    if (dmac_region(DGBRP(chan)) == dmac_region(DGBWP(chan)))
        dmac_region(DGCS(chan)) &= ~DGCS_BNE; /* buffer is empty */
    else
        dmac_region(DGCS(chan)) |= DGCS_BNE; /* buffer not empty */

    /* buffer wrap ? */
    if (dmac_region(DGBRP(chan)) >= dmac_region(DGBS(chan)))
      dmac_region(DGBRP(chan)) =  dmac_region(DGBBA(chan));
}

static void chan_update_wptr(struct adsp_hda_dmac *dmac, int chan, int bytes)
{
    dmac_region(DGBWP(chan)) += bytes;

    /* check R ptr wrt to W ptr */
    if (dmac_region(DGBRP(chan)) == dmac_region(DGBWP(chan)))
        dmac_region(DGCS(chan)) |= ~DGCS_BF; /* buffer is full */
    else
        dmac_region(DGCS(chan)) &= ~DGCS_BF; /* buffer not full */

    /* buffer wrap ? */
    if (dmac_region(DGBWP(chan)) >= dmac_region(DGBS(chan)))
      dmac_region(DGBWP(chan)) =  dmac_region(DGBBA(chan));
}
...
static int dma_M2P_copy_burst(struct hda_dma_chan *hda_dma_chan)
{
    ...
    /* no data ? */
    if (!(dmac_region(DGCS(chan)) & DGCS_BNE))
        return 1;

    sar = dmac_region(DGBRP(chan));
    size = dmac_region(DGBFPI(chan));
    burst_size = size / 2;
    ...
    //trace_hda_dma_m2p_copy(dmac->id, chan, sar, dmac_region(DGBWP(chan)),
    //    size, hda_dma_chan->tbytes);
...
static int dma_P2M_copy_burst(struct hda_dma_chan *hda_dma_chan)
{
    ...
    /* buffer full ? */
    if (!(dmac_region(DGCS(chan)) & DGCS_BF))
        return 1;

    dar = dmac_region(DGBWP(chan));
    size = dmac_region(DGBFPI(chan));
    burst_size = size / 2;
  ...
  //      dmac->id, chan, dar, dmac_region(DGBRP(chan)),
  //      size,
   //     hda_dma_chan->tbytes);
...
static int dma_M2M_read_host_burst(struct hda_dma_chan *hda_dma_chan)
{
  ...
  //      dmac->id, chan, dmac_region(DGBRP(chan)), dar,
  //      dmac_region(DGBFPI(chan)),
  //      hda_dma_chan->tbytes);
...
static int dma_M2M_write_host_burst(struct hda_dma_chan *hda_dma_chan)
{
 ...
 //   log_text(dmac->log, LOG_DMA_M2M,
 //       "dma: %d:%d: completed SAR 0x%x DAR 0x%x size 0x%x total bytes 0x%x\n",
 //       dmac->id, chan, sar, dmac_region(DGBWP(chan)),
 //       dmac_region(DGBFPI(chan)),
 //       hda_dma_chan->tbytes);
...
static void dma_stop_transfer(struct adsp_hda_dmac *dmac, uint32_t chan)
{
  ...
  //      dmac->id, chan, dmac_region(DGBRP(chan)), dmac_region(DGBWP(chan)),
  //      dmac_region(DGBFPI(chan)),
 //       hda_dma_chan->tbytes);
}
