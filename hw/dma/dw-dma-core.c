...
#define dmac_region(daddr)  dmac->io[daddr >> 2]

/* mimic the time it takes for the DMA to copy data */
...
static void dmac_reg_sync(struct adsp_gp_dmac *dmac, hwaddr addr)
{
    ...
    case DW_STATUS_BLOCK:
        val = 0xff & dmac_region(DW_RAW_BLOCK) & ~dmac_region(DW_MASK_BLOCK);
        dmac_region(DW_STATUS_BLOCK) = val;
        dmac_reg_sync(dmac, DW_INTR_STATUS);
        break;
    case DW_STATUS_TFR:
        val = 0xff & dmac_region(DW_RAW_TFR) & ~dmac_region(DW_MASK_TFR);
        dmac_region(DW_STATUS_TFR) = val;
        dmac_reg_sync(dmac, DW_INTR_STATUS);
        break;
    case DW_INTR_STATUS:
        if (dmac_region(DW_STATUS_BLOCK) & 0xff)
            dmac_region(DW_INTR_STATUS) |= DW_INTR_STATUS_BLOCK;
        else
            dmac_region(DW_INTR_STATUS) &= ~DW_INTR_STATUS_BLOCK;
        if (dmac_region(DW_STATUS_TFR) & 0xff)
            dmac_region(DW_INTR_STATUS) |= DW_INTR_STATUS_TFR;
        else
            dmac_region(DW_INTR_STATUS) &= ~DW_INTR_STATUS_TFR;

  //      log_text(dmac->log, LOG_DMA_IRQ,
   //         "IRQ: from DMAC %d status %x block %x tfr %x\n", dmac->id,
   //         dmac_region(DW_INTR_STATUS),
   //         dmac_region(DW_STATUS_BLOCK),
   //         dmac_region(DW_STATUS_TFR));

        if (dmac_region(DW_INTR_STATUS) && !dmac->irq_assert) {
            dmac->irq_assert = 1;

       //     log_text(dmac->log, LOG_DMA_IRQ, "IRQ: DMAC %d set\n",
           //     dmac->id);

            dmac->do_irq(dmac, 1, dmac_region(DW_INTR_STATUS));

        } else if (dmac_region(DW_INTR_STATUS) == 0 && dmac->irq_assert) {
            dmac->irq_assert = 0;

      //      log_text(dmac->log, LOG_DMA_IRQ,"IRQ: DMAC %d clear\n",
       //         dmac->id);
            dmac->do_irq(dmac, 0, dmac_region(DW_INTR_STATUS));
        }
        break;
    default:
        break;
    ...
    uint32_t direction)
{
    ...
    dma_msg->host_data = 0;
    dma_msg->src = dmac_region(DW_SAR(chan));
    dma_msg->dest = dmac_region(DW_DAR(chan));
    dma_msg->size = dmac_region(DW_CTRL_HIGH(chan)) & DW_CTLH_BLOCK_TS_MASK;
...
static int dma_llp_reloaded(struct dma_chan *dma_chan)
{
    ...
    /* pointer to next LLP */
    if (dmac_region(DW_LLP(chan)) == 0 || dma_chan->stop)
        return 0;

    /* get next LLP */
    lli = cpu_physical_memory_map(dmac_region(DW_LLP(chan)), &lli_size, 0);
    if (lli == NULL) {
        printf("cant map LLP at 0x%x\n", dmac_region(DW_LLP(chan)));
        return 0;
    }

    /* update SAR */
    src_llp_en = (dmac_region(DW_CTRL_LOW(chan)) & DW_CTLL_LLP_S_EN) != 0;
    if (src_llp_en)
        dmac_region(DW_SAR(chan)) = lli->sar;

    /* update DAR */
    dst_llp_en = (dmac_region(DW_CTRL_LOW(chan)) & DW_CTLL_LLP_D_EN) != 0;
    if (dst_llp_en)
        dmac_region(DW_DAR(chan)) = lli->dar;

    /* update LLP */
    dmac_region(DW_LLP(chan)) = lli->llp;

    /* update CTL_LO */
    dmac_region(DW_CTRL_LOW(chan)) = lli->ctrl_lo;

    /* update CTL_HI */
    dmac_region(DW_CTRL_HIGH(chan)) = lli->ctrl_hi;
 ...
 //       dmac_region(DW_SAR(chan)), dmac_region(DW_DAR(chan)),
 //       dmac_region(DW_CTRL_HIGH(chan)) & DW_CTLH_BLOCK_TS_MASK);
...
static int dma_M2P_copy_burst(struct dma_chan *dma_chan)
{
    ...
    sar = dmac_region(DW_SAR(chan));
    size = dmac_region(DW_CTRL_HIGH(chan)) & DW_CTLH_BLOCK_TS_MASK;
    burst_size = size / 2;
    ...
    dmac_region(DW_SAR(chan)) += burst_size;
    ...
    if (dma_chan->bytes >= size || dma_chan->stop) {
        ...
        dmac_region(DW_RAW_BLOCK) |= CHAN_RAW_ENABLE(chan);
        ...
        if (dma_llp_reloaded(dma_chan) && !dma_chan->stop) {
            ...
            log_text(dmac->log, LOG_DMA_M2P,
                ...
                dmac->id, chan, sar, dmac_region(DW_DAR(chan)),
                dmac_region(DW_CTRL_HIGH(chan)) & DW_CTLH_BLOCK_TS_MASK,
                dma_chan->tbytes);
            ...
            dmac_region(DW_DMA_CHAN_EN) &= ~CHAN_RAW_ENABLE(chan);

            /* assert tfr interrupt */
            dmac_region(DW_RAW_TFR) |= CHAN_RAW_ENABLE(chan);
...
static int dma_P2M_copy_burst(struct dma_chan *dma_chan)
{
    ...
    /* update SAR, DAR and bytes copied */
    dmac_region(DW_DAR(chan)) += burst_size;
    dma_chan->ptr += burst_size;
    dma_chan->bytes += burst_size;
    dma_chan->tbytes += burst_size;
    ...
    if (dma_chan->bytes >= size || dma_chan->stop) {
        ...
        if (dma_llp_reloaded(dma_chan) && !dma_chan->stop) {
            ...
            log_text(dmac->log, LOG_DMA_P2M,
                "dma: %d:%d: completed SAR 0x%lx DAR 0x%lx size 0x%x total bytes 0x%x\n",
                dmac->id, chan, dmac_region(DW_SAR(chan)), dar,
                dmac_region(DW_CTRL_HIGH(chan)) & DW_CTLH_BLOCK_TS_MASK,
                dma_chan->tbytes);
...
//        dmac->id, chan, dmac_region(DW_SAR(chan)), dmac_region(DW_DAR(chan)),
//        dmac_region(DW_CTRL_HIGH(chan)) & DW_CTLH_BLOCK_TS_MASK,
 //       dma_chan->tbytes);
}
...
static void dma_start_transfer(struct adsp_gp_dmac *dmac, uint32_t chan)
{
  ...
  //  uint32_t dar;
   ...
   // dar = dmac_region(DW_DAR(chan));
    sar = dmac_region(DW_SAR(chan));
    ctl_lo = dmac_region(DW_CTRL_LOW(chan));
        ...
        uint64_t val, unsigned size)
{
  ...
  //  const struct adsp_reg_space *gp_dmac_dev = dmac->desc;
    ...
    if ((addr != DW_DMA_CFG) && !(dmac_region(DW_DMA_CFG) & 1))
        return;

    set = val & ~dmac_region(addr);
 ...
 //           dmac_region(addr));
    ...
    case DW_RAW_TFR:
        ...
        break;
    case DW_MASK_BLOCK:
        active = dmac_region(DW_MASK_BLOCK) & 0xff;
        ...
        dmac_region(DW_MASK_BLOCK) = active;
        dmac_reg_sync(dmac, DW_STATUS_BLOCK);
        break;
    case DW_MASK_TFR:
        active = dmac_region(DW_MASK_TFR) & 0xff;
        ...
        dmac_region(DW_MASK_TFR) = active;
        dmac_reg_sync(dmac, DW_STATUS_TFR);
        break;
    case DW_CLEAR_BLOCK:
        dmac_region(addr) = val;
        dmac_region(DW_RAW_BLOCK) &= ~val;
        dmac_reg_sync(dmac, DW_STATUS_BLOCK);
        break;
    case DW_CLEAR_TFR:
        dmac_region(addr) = val;
        dmac_region(DW_RAW_TFR) &= ~val;
        dmac_reg_sync(dmac, DW_STATUS_TFR);
        break;
    case DW_STATUS_TFR:
    case DW_STATUS_BLOCK:
